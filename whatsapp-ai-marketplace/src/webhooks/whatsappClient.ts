import type { BotReply } from "../bot/types.js";
...
export async function sendWhatsAppReply(to: string, reply: BotReply) {
...
function formatReply(reply: BotReply): string {
...
async function sendAudioReply(to: string, text: string) {
  ...
  const payload: WhatsAppAudioPayload = {
    messaging_product: "whatsapp",
    recipient_type: "individual",
    to,
    type: "audio",
...
async function uploadWhatsAppAudio(audio: ArrayBuffer): Promise<string | undefined> {
