...
import type { BotReply, BotTurn, IncomingBotMessage, LanguageCode, UserProfile } from "./types.js";
...
export async function handleOnboardingTurn(profile: UserProfile, message: IncomingBotMessage): Promise<BotTurn> {
  ...
  if (profile.onboardingStage === "ASKING_GOAL") {
    ...
    const updatedProfile = { ...profile, ...goal, onboardingStage: "COMPLETE" as const };
    return {
...
function getMessageText(message: IncomingBotMessage): string {
...
function parseLanguage(text: string): LanguageCode | undefined {
...
function languagePrompt(): BotReply {
  return {
...
function askTrade(language: LanguageCode = "english"): BotReply {
  const selectedCopy = copy(language);
  return { text: selectedCopy.askTrade(LANGUAGES[language]) };
}

function askLocation(trade?: string, guessedLocation?: string, language: LanguageCode = "english"): BotReply {
  return { text: copy(language).askLocation(trade, guessedLocation) };
}

function askGoal(trade?: string, location?: string, language: LanguageCode = "english"): BotReply {
  return goalPrompt(trade, location, language);
}

function goalPrompt(trade?: string, location?: string, language: LanguageCode = "english"): BotReply {
...
function audioTranscriptMissingReply(language: LanguageCode = "english"): BotReply {
  return { text: copy(language).audioTranscriptMissing };
}

function parseGoal(text: string):
  | { intent: "JOB_SEEKER"; wantsJobs: true; wantsBusinessHelp: false }
  | { intent: "BUSINESS_GROWTH"; wantsJobs: false; wantsBusinessHelp: true }
  | undefined {
...
function completeReply(profile: UserProfile): BotReply {
  return { text: copy(profile.language).complete(profile) };
}

function postOnboardingReply(profile: UserProfile, message: IncomingBotMessage): BotReply {
...
function copy(language: LanguageCode = "english") {
  if (language === "pidgin") {
    return {
      askTrade: (_languageName: string) =>
        "Correct. We go continue for Pidgin.\n\nWetin you dey do? You fit type am for here.\n\nExample: I sell tomatoes in Mile 12",
    askLocation: (trade?: string, guessedLocation?: string) =>
        `${guessedLocation ? `I hear say na ${guessedLocation}. ` : ""}Where you dey stay or where your business dey${trade ? ` for ${trade}` : ""}?`,
      goalPrompt: (summary?: string) =>
        `${summary ? `I don get am: ${summary}.\n\n` : ""}Wetin you want make I help you with?`,
      tradeRetry:
        "Tell me wetin you dey do. For now, type am here.\n\nExample: I am a tailor in Yaba",
      locationRetry:
        "I no understand the location. Abeg type where you dey stay or where your business dey.\n\nExample: I stay in Yaba",
      audioTranscriptMissing:
        "I received your voice note, but voice-to-text never connect yet. Abeg type wetin you talk for now.\n\nExample: I sell tomatoes in Mile 12",
      complete: (profile: UserProfile) =>
        `Done. I don create your profile.\n\nName: ${profile.displayName ?? "Your WhatsApp name"}\nProfession: ${profile.trade ?? "Not set"}\nLocation: ${profile.location ?? "Not set"}\nGoal: ${profile.wantsJobs ? "Find jobs" : "Grow business"}\n\nNext: send photo of your goods/work, type "jobs", or type "business".`,
      dashboard: (profile: UserProfile) =>
        `Welcome back${profile.displayName ? `, ${profile.displayName}` : ""}.\n\nProfession: ${profile.trade ?? "Not set"}\nLocation: ${profile.location ?? "Not set"}\n\nSend photo of your goods/work, type "jobs", or type "business".`,
      photoReceived: (profile: UserProfile) =>
        `I don receive the photo. I go attach am to your ${profile.trade ?? "business"} profile. You fit send another photo, type "jobs", or type "business".`,
      jobs: (profile: UserProfile) =>
        `Jobs noted. I go use your profile: ${profile.trade ?? "profession"} for ${profile.location ?? "your area"} to match opportunities. Send your skill, price/rate, or any certificate if you get.`,
      businessHelp: (profile: UserProfile) =>
        `Business help noted. For ${profile.trade ?? "your business"} in ${profile.location ?? "your area"}, send photo of what you sell/work you do, your price range, and the kind customer you want.`,
    };
  }
  ...
  if (language === "yoruba") {
    return localizedCopy({
      ...
      askLocation: (trade, guessedLocation) =>
        `${guessedLocation ? `Mo gbo pe o wa ni ${guessedLocation}. ` : ""}Nibo ni o ngbe tabi nibo ni ise re wa${trade ? ` fun ${trade}` : ""}?`,
  ...
  if (language === "hausa") {
    return localizedCopy({
      ...
      askLocation: (trade, guessedLocation) =>
        `${guessedLocation ? `Na ji kana ${guessedLocation}. ` : ""}Ina kake zama ko ina kasuwancinka yake${trade ? ` na ${trade}` : ""}?`,
  ...
  if (language === "igbo") {
    return localizedCopy({
      ...
      askLocation: (trade, guessedLocation) =>
        `${guessedLocation ? `Anuru m na i no na ${guessedLocation}. ` : ""}Ebee ka i bi ma obu ebee ka business gi di${trade ? ` maka ${trade}` : ""}?`,
  ...
  return {
    ...
    askLocation: (trade?: string, guessedLocation?: string) =>
      `${guessedLocation ? `I heard ${guessedLocation}. ` : ""}Where do you stay or where is your business located${trade ? ` for ${trade}` : ""}?`,
    goalPrompt: (summary?: string) =>
      `${summary ? `I got: ${summary}.\n\n` : ""}Do you want jobs or do you want to grow your business?`,
      tradeRetry:
      "Tell me what you do. For now, type it here.\n\nExample: I am a tailor in Yaba",
    locationRetry:
      "I did not understand the location. Please type where you stay or where your business is located.\n\nExample: I stay in Yaba",
    audioTranscriptMissing:
      "I received your voice note, but voice-to-text is not connected yet. Please type what you said for now.\n\nExample: I sell tomatoes in Mile 12",
    complete: (profile: UserProfile) =>
      `Done. I created your profile.\n\nName: ${profile.displayName ?? "Your WhatsApp name"}\nProfession: ${profile.trade ?? "Not set"}\nLocation: ${profile.location ?? "Not set"}\nGoal: ${profile.wantsJobs ? "Find jobs" : "Grow business"}\n\nNext: send photos of your goods/work, type "jobs", or type "business".`,
    dashboard: (profile: UserProfile) =>
      `Welcome back${profile.displayName ? `, ${profile.displayName}` : ""}.\n\nProfession: ${profile.trade ?? "Not set"}\nLocation: ${profile.location ?? "Not set"}\n\nSend photos of your goods/work, type "jobs", or type "business".`,
    photoReceived: (profile: UserProfile) =>
      `Photo received. I will attach it to your ${profile.trade ?? "business"} profile. You can send another photo, type "jobs", or type "business".`,
    jobs: (profile: UserProfile) =>
      `Jobs noted. I will use your profile: ${profile.trade ?? "profession"} in ${profile.location ?? "your area"} to match opportunities. Send your skill details, expected pay, or certificate if you have one.`,
    businessHelp: (profile: UserProfile) =>
      `Business help noted. For ${profile.trade ?? "your business"} in ${profile.location ?? "your area"}, send photos, your price range, and the kind of customers you want.`,
  };
}

function localizedCopy(config: {
  askTrade: string;
  askLocation: (trade?: string, guessedLocation?: string) => string;
  goalPrompt: (summary?: string) => string;
  retry: string;
  audio: string;
  done: string;
  dashboard: string;
  ...
  return {
    ...
    photoReceived: (profile: UserProfile) =>
      `Photo received for ${profile.trade ?? "your profile"}. Send another photo, type "jobs", or type "business".`,
    jobs: (profile: UserProfile) =>
      `Jobs noted for ${profile.trade ?? "your profession"} in ${profile.location ?? "your area"}. Send skill details, expected pay, or certificate if available.`,
    businessHelp: (profile: UserProfile) =>
      `Business help noted for ${profile.trade ?? "your business"} in ${profile.location ?? "your area"}. Send photos, price range, and target customers.`,
  };
}
