...
import type { ProfileUnderstanding } from "./types.js";
...
export async function understandUserProfileText(text: string): Promise<ProfileUnderstanding> {
...
function fallbackUnderstanding(text: string, allowSimpleProfession: boolean): ProfileUnderstanding {
  const extracted = extractProfileFromText(text);
  const hasWorkSignal = /\b(i\s+am|i'm|im|am\s+a|am\s+an|i\s+sell|selling|i\s+do|i\s+make|i\s+repair|i\s+fix|i\s+build|i\s+cook|i\s+drive|i\s+work|profession|occupation|skill|business|service|trade|job)\b/i.test(
    text,
  );
...
function normalizeUnderstanding(value: Record<string, unknown>): ProfileUnderstanding {
...
function parseIntent(value: unknown): ProfileUnderstanding["intent"] {
